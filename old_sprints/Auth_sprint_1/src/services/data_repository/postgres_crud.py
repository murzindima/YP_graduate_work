        ...
        async with self.session.begin():
            ...
            if model_data:
                await self.session.execute(
                    update(self.model_class).where(self.model_class.id == model_id).values(model_data)
                )
    ...
    async def delete(self, model_id: UUID) -> M | None:
        ...
        async with self.session.begin():
            result = await self.session.execute(
                delete(self.model_class).where(self.model_class.id == model_id).returning(self.model_class)
            )
