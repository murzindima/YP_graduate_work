...
class UserService(BaseService):
    ...
    async def get_login_history(self, user_id: UUID, page: int, size: int) -> list[LoginHistorySchema]:
        """Validate user's login history."""
        db_models = await self.postgres_service.get_login_history(user_id=user_id, page=page, size=size)
    ...
    async def create_user(self, new_user: UserCreateSchema) -> UserSchema:
        ...
        if existing_user:
            raise HTTPException(detail=USER_ALREADY_EXISTS, status_code=status.HTTP_401_UNAUTHORIZED)

        requested_role = await self.postgres_service.get_role(role_id=new_user.role_id)
        if not requested_role:
            raise HTTPException(detail=ROLE_NOT_FOUND, status_code=status.HTTP_404_NOT_FOUND)
    ...
    async def update_user(self, user_id: UUID, new_user_data: UserUpdateSchema) -> UserSchema | None:
        """Update an existing user."""
        if new_user_data.role_id:
            requested_role = await self.postgres_service.get_role(role_id=new_user_data.role_id)
            if not requested_role:
                raise HTTPException(detail=ROLE_NOT_FOUND, status_code=status.HTTP_404_NOT_FOUND)
