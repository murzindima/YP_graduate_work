...
def check_connection(ping_function, service_name, max_retries=10, initial_sleep_interval=3, is_async=False):
    ...
    for attempt in range(max_retries):
        try:
            ...
            logging.warning(f"Attempt {attempt + 1} to connect to {service_name} failed: {e}")
        time.sleep(sleep_interval)
        sleep_interval *= 2

    logging.error(f"Maximum retry attempts reached. Could not connect to {service_name}.")
    return False
