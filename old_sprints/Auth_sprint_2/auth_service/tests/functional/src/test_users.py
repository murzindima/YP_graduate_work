...
@pytest.mark.asyncio
async def test_get_user_by_random_member(api_base_url, get_jwt_tokens_member, create_user):
    user_id, role_id, _ = create_user
    access_token, _ = get_jwt_tokens_member
    headers = {"Authorization": "Bearer " + access_token}
    async with ClientSession(headers=headers) as session:
...
async def test_get_user_by_member(api_base_url, create_user):
    ...
    async with ClientSession() as session:
        ...
        async with session.post(url, json={"email": email, "password": MEMBER_PASSWORD}) as response:
            body = await response.json()
...
@pytest.mark.asyncio
async def test_patch_user_wrong_role_id(api_base_url, get_jwt_tokens_admin, create_user):
    user_id, _, _ = create_user
    access_token, _ = get_jwt_tokens_admin
    headers = {"Authorization": "Bearer " + access_token}
    async with ClientSession(headers=headers) as session:
...
@pytest.mark.asyncio
async def test_patch_user_refresh_token(api_base_url, get_jwt_tokens_admin, create_user):
    _, refresh_token = get_jwt_tokens_admin
    user_id, _, _ = create_user
    headers = {"Authorization": "Bearer " + refresh_token}
    async with ClientSession(headers=headers) as session:
