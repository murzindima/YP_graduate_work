...
@router.post(
    "/signup", response_model=UserSchema, status_code=status.HTTP_201_CREATED, dependencies=[Depends(AccessJWTBearer())]
)
...
@router.post(
    "/login", response_model=TokenPairSchema, status_code=status.HTTP_200_OK, dependencies=[Depends(AccessJWTBearer())]
)
async def login(
...
@router.post("/logout", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(set_current_user_access_jwt)])
async def logout(
    request: Request,
    auth_service: AuthService = Depends(get_auth_service),
) -> None:
...
async def oauth_login(provider: str, auth_service: AuthService = Depends(get_auth_service)):
...
@router.get("/login/oauth/{provider}/callback", response_model=TokenPairSchema, status_code=status.HTTP_200_OK)
async def oauth_callback(provider: str, request: Request, auth_service: AuthService = Depends(get_auth_service)):
